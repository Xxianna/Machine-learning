...
int main(){
    ...
    try{
        ...
        int sz1[2]={1,2},sz2[2]={1,9},sz3[2]={1,1},sz4[2]={1,9};
        Network net;net0=&net;
        Tensor t1(2,sz1),t2(2,sz2),t3(2,sz3),t4(2,sz4);
        BiasLayer b1(t2),b2(t3),b3(t4);
        LinearLayer l1(t1,t2),l2(t2,t4),l3(t4,t3);
        ActivationLayer a1(tanh_info),a2(sigmoid_info);
        net.add_layer(&l1).add_layer(&b1).add_layer(&a1).add_layer(&l2).add_layer(&b3);
        net.add_layer(&a2).add_layer(&l3).add_layer(&b2);
        Linearloss loss;//直接求差作为损失函数
        Momentum opt;//加入动量的梯度下降
        opt.set_decay(0.9996).set_learning_rate(0.004).set_momentum(0.6);
